...
def all_vowel_pairs(words)
    ...
    words.each_with_index do |word1, idx| 
        words[idx+1..-1].each do |word2|
            ...
            all_vowels = vowels.all? { |vowel| combined_word.include?(vowel) }
            pairs << combined_word if all_vowels 
        end 
    end 
...
class String
    ...
    def substrings(length = nil)
        ...
        (0..self.length-1).each do |idx1|
            if length
                substrings << self[idx1...idx1+length] if self[idx1...idx1+length].length == length
            else
